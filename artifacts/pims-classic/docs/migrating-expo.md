# Migrating Expo UI to PIMS Classic

Read `artifacts/pims-classic/docs/AGENTS.md` and
`artifacts/pims-classic/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/pims-classic/lib/native-theme` and
  `@workspace/pims-classic/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/pims-classic/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/pims-classic` and retains only
