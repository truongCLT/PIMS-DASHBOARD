# Migrating Expo UI to 대우본사 요청-예시1

Read `artifacts/daewoo-hq-example-1/docs/AGENTS.md` and
`artifacts/daewoo-hq-example-1/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/daewoo-hq-example-1/lib/native-theme` and
  `@workspace/daewoo-hq-example-1/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/daewoo-hq-example-1/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/daewoo-hq-example-1` and retains only
