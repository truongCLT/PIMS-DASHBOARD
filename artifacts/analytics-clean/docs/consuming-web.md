# Consuming Analytics Clean in web apps

Read `artifacts/analytics-clean/docs/AGENTS.md` first. This guide covers
...
`artifacts/analytics-clean/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/analytics-clean/styles.css";
```
...
`node_modules/@workspace/analytics-clean/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/analytics-clean/components/ui/button";
import { cn } from "@workspace/analytics-clean/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/analytics-clean/hooks/use-toast";
```
...
`@workspace/analytics-clean/components/ui/button`. Run the app's typecheck
