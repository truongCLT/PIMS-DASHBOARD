# Migrating Expo UI to Analytics Clean

Read `artifacts/analytics-clean/docs/AGENTS.md` and
`artifacts/analytics-clean/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/analytics-clean/lib/native-theme` and
  `@workspace/analytics-clean/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/analytics-clean/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/analytics-clean` and retains only
