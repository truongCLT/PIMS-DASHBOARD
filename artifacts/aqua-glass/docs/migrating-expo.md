# Migrating Expo UI to Aqua Glass

Read `artifacts/aqua-glass/docs/AGENTS.md` and
`artifacts/aqua-glass/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/aqua-glass/lib/native-theme` and
  `@workspace/aqua-glass/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/aqua-glass/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/aqua-glass` and retains only
