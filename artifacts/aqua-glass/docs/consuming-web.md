# Consuming Aqua Glass in web apps

Read `artifacts/aqua-glass/docs/AGENTS.md` first. This guide covers
...
`artifacts/aqua-glass/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/aqua-glass/styles.css";
```
...
`node_modules/@workspace/aqua-glass/src/components` to `content`.
...
import { cn } from "@workspace/aqua-glass/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/aqua-glass/hooks/use-toast";
```
...
`@workspace/aqua-glass/components/ui/button`. Run the app's typecheck
