# Consuming 대우본사요청-예시2 in web apps

Read `artifacts/daewoo-hq-example-2/docs/AGENTS.md` first. This guide covers
...
`artifacts/daewoo-hq-example-2/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/daewoo-hq-example-2/styles.css";
```
...
`node_modules/@workspace/daewoo-hq-example-2/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/daewoo-hq-example-2/components/ui/button";
import { cn } from "@workspace/daewoo-hq-example-2/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/daewoo-hq-example-2/hooks/use-toast";
```
...
`@workspace/daewoo-hq-example-2/components/ui/button`. Run the app's typecheck
