...
Additional trap: the testing subagent's browser can get redirected from `localhost:80/<path>/` to the `$REPLIT_DEV_DOMAIN` **root**, silently dropping the path and landing on the root app even though the plan said localhost. Fix: give the tester the full URL `https://$REPLIT_DEV_DOMAIN/<path>/` explicitly — guard + reload loops on localhost do NOT recover from this.
