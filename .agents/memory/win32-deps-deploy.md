...
**How to apply:** If a deploy build fails at "Installing packages" with EBADPLATFORM, grep package.json files for `win32`/`-msvc`/`darwin` direct deps, remove them, and run `pnpm install --lockfile-only`. A guard task (#104) was proposed to automate this check.
