...
- 경영관리보고회 web upload: parser/apply in `artifacts/api-server/src/lib/mgmtreportImport.ts` (mirrors the CLI importer — keep both in sync), routes POST `/mgmtreport/import/preview|apply` (multipart file+year, full replace), UI modal `artifacts/pims-dashboard-2/src/components/MgmtReportUploadModal.tsx` opened from the "Excel 업로드" button in `DashboardHeader.tsx`
